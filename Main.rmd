...
fitBayesianCAPM = function(model_data, factors, fileName, write) {
  ...
  # Fitting Bayesian model -> RETHINK PRIOR SPECIFICATION | ALSO: CONSIDER MODEL SPECIFICATION (GIBBS?)
...
# Parallel importance reasmpling function
...
#####3 Test new function with new definition of MKT #######
...
# Three-Factor + RMW
model_formula = c("Lo30Ex", "MKTRF", "SMB", "HML", "RMW")
model_3ff_rmw = fitBayesianCAPM(ff_data, model_formula, fileName = "data/object/w_matrix_3ff_rmw_lo30.rds", write = FALSE)

# Three-Factor + CMA
model_formula = c("Lo30Ex", "MKTRF", "SMB", "HML", "CMA") 
model_3ff_cma = fitBayesianCAPM(ff_data, model_formula, fileName = "data/object/w_matrix_3ff_cma_lo30.rds", write = FALSE)
...
plotPCA(ff_data, model_3ffk[[1]], "Three-Factor CAPM Small Stocks")

temp.data = data.frame(Time = as.numeric(substr(ff_data$Time, 0, 4)) + as.numeric(substr(ff_data$Time, 5, 6))/12, Lo30Ex = ff_data$Lo30Ex)
ggplot(temp.data, aes(x = Time, y = Lo30Ex)) + geom_line()

# plotPCA(ff_data, model_5ff[[1]], "Five-Factor CAPM Small Stocks")
# plotPCA(ff_data, model_3ff_cma[[1]], "3FF + CMA")
# plotPCA(ff_data, model_3ff_rmw[[1]], "3FF + RMW")
...
p1 = ggplot(y_hat_med[1:350,], aes(x = Time, y = median, colour = Influential)) + 
  ...
  scale_color_discrete(name = "Group", labels = c("A", "B")) + ylab("Difference") + theme(legend.position = c(0.9, 0.2))
...
ggplot(transform(marginal_diff.long,
  ...
  facet_wrap(~param, nrow = 2, ncol = 2, labeller = label_parsed) + ylab("Difference")
graphics.off()
```
...
p1 = ggplot(y_hat_plot, aes(x = Time, y = median, colour = group)) + 
  geom_errorbar(aes(ymin = low25, ymax = high75), width = 0.14, size = 0.2) +
  geom_point(shape = 18) + ylab("Difference") + xlab("Observation") + geom_hline(yintercept = 0, alpha = 0.5) +
